...
params:
  ...
  data2: "./Data/WBC.csv"
  ptrain: !r 2/3
  seed_train:  805547 #378528
  seed_alg: 713945 #303381   
---
...
head(prediction)
```
## Confusion matrix ANN Small Dataset
...
# Información para construir la Tabla 1
```{r}
print("KNN")
cm_knn$overall[1]
cm_knn$overall[2]
...
cm_ann_s$byClass
```
# Figura 4. Comparación de medidas de tiempo y exactitud para kNN, SVM y RF
...
#cm_knn
 ...
 times <- c(knn_time, svm_time, rf_time)
 
 color <- c("dodgerblue", "lightskyblue", "cyan")
 
 Acc <- c(cm_knn$overall[1], cm_svm$overall[1], cm_rf$overall[1])
   
 Kappa <- c(cm_knn$overall[2], cm_svm$overall[2], cm_rf$overall[2])
 
 Sensib <- c(cm_knn$byClass[1], cm_svm$byClass[1], cm_rf$byClass[1])
 
 Espec <- c(cm_knn$byClass[2], cm_svm$byClass[2], cm_rf$byClass[2])
 
 PPV <- c(cm_knn$byClass[3], cm_svm$byClass[3], cm_rf$byClass[3])
 
 NPV <- c(cm_knn$byClass[4], cm_svm$byClass[4], cm_rf$byClass[4])
 
 F1 <- c(cm_knn$byClass[7], cm_svm$byClass[7], cm_rf$byClass[7])
   
 AUC <- c(0.989, 0.996, 0.992)
 
 time_p <- barplot(as.vector(times),names.arg=c("kNN", "SVM", "RF"), 
                   col = color,ylab=c("Proporcion"), main="Tiempo") 
                   #,ylim=c(0,70)
 
 text(x = time_p, y = c(times), label = round(times,3), pos = 1, 
      cex = 0.8, col = "black")
 ...
 plot.roc(model_knn$pred$obs,
          ...
          col="blue", main = "Curva ROC para kNN")
 ...
 plot.roc(model_svm$pred$obs,
          ...
          max.auc.polygon=TRUE, grid=TRUE,
          print.auc=TRUE, show.thres=TRUE,
          col="blue", main = "Curva ROC para SVM")
...
#           max(model$results$Kappa), 
#           max(rf_caret_model$results$Kappa))
color <- c("dodgerblue","royalblue", "navyblue")
...
time_p <- barplot(as.vector(times_3), names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), ylim=c(0,35), main="Time", cex.main=1.5)
text(x = time_p, y = times, label = round(times,3), pos = 3, cex = 1.1, col = "black")
Acc_p <- barplot(Acc, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Accuracy", cex.main=1.5)
text(x = Acc_p, y = Acc-0.05, label = round(Acc,4), pos = 1, cex = 1.1, col = "white")
K_p <- barplot(Kappa, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Kappa", cex.main=1.5)
text(x = K_p, y = Kappa-0.05, label = round(Kappa,4), pos = 1, cex = 1.1, col = "white")
Sens_p <- barplot(Sensib, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Sensititivy", cex.main=1.5)
text(x = Sens_p, y = Sensib-0.05, label = round(Sensib,4), pos = 1, cex = 1.1, col = "white")
Espe_p <- barplot(Espec, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Specificity", cex.main=1.5)
text(x = Espe_p, y = Espec-0.05, label = round(Espec,4), pos = 1, cex = 1.1, col = "white")
PPV_p <- barplot(PPV, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Positive Predictive Value", cex.main=1.5)
text(x = PPV_p, y = PPV-0.05, label = round(PPV,4), pos = 1, cex = 1.1, col = "white")
NPV_p <- barplot(NPV, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="Negative Predictive Value", cex.main=1.5)
text(x = NPV_p, y = NPV-0.05, label = round(NPV,4), pos = 1, cex = 1.1, col = "white")
F1_p <- barplot(F1, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="F1", cex.main=1.5)
text(x = F1_p, y = F1-0.05, label = round(F1,4), pos = 1, cex = 1.1, col = "white")
AUC_p <- barplot(AUC, names.arg=c("kNN", "SVM", "RF"), col = color,
        ylab=c("Proportion"), main="AUC", cex.main=1.5)
text(x = AUC_p, y = AUC-0.05, label = round(AUC,4), pos = 1, cex = 1.1, col = "white")
...
plot.roc(model_knn_s$pred$obs,
         ...
         max.auc.polygon=TRUE, grid=TRUE,
         print.auc=TRUE, show.thres=TRUE, 
         col="dodgerblue", main = "ROC Curve for kNN")
plot.roc(model_svm_s$pred$obs,
         model_svm_s$pred$Malignant,
         smoothed = TRUE,
         legacy.axes=TRUE,
